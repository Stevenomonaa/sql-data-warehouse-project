...
SELECT
    ...
    CASE
        WHEN current_year_sales > LAG(current_year_sales) OVER (PARTITION BY product_name ORDER BY sales_year) THEN 'Increase' 
        WHEN current_year_sales < LAG(current_year_sales) OVER (PARTITION BY product_name ORDER BY sales_year) THEN 'Decrease'
        ELSE 'No Change'
