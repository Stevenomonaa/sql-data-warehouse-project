...
Script Purpose:
    ...
    - Truncates Silver tables prior to loading to ensure idempotent execution.
    - Cleans, standardizes, and normalizes data from Bronze sources.
    - Applies business rules and data quality corrections.
    - Loads ERP master data first to establish parent entities.
    - Promotes missing category records referenced by CRM products into
      silver.erp_px_cat_g1v2 using inferred (placeholder) values.
    - Enriches promoted category records with standardized default attributes
      (e.g., 'Unknown') to preserve referential integrity.
    - Loads CRM product data after category promotion to ensure valid parent-child
      relationships.
    - Loads CRM transactional data only after all required master data exists.
    - Supports late-arriving dimensions by allowing category enrichment in
      subsequent loads.
...
BEGIN
    ...
    ----------------------------------------------------------------------------
    -- ERP PRODUCT CATEGORY (PARENT + PROMOTION TARGET)
    ----------------------------------------------------------------------------
    RAISE NOTICE '>> Loading silver.erp_px_cat_g1v2 (ERP categories)';
    TRUNCATE TABLE silver.erp_px_cat_g1v2;

    -- Load categories from ERP
    ...
    ----------------------------------------------------------------------------
    -- PROMOTE MISSING CATEGORIES FROM CRM (STRUCTURAL TRUTH)
    ----------------------------------------------------------------------------
    RAISE NOTICE '>> Promoting missing categories from CRM';
    ...
    WHERE NOT EXISTS (
        ...
        WHERE c.id = REPLACE(SUBSTRING(p.prd_key,1,5),'-','_')
    );

    ----------------------------------------------------------------------------
    -- ENRICHMENT (KNOWN FIXES / BUSINESS RULES)
    ----------------------------------------------------------------------------
    RAISE NOTICE '>> Enriching known categories';
    ...
    RAISE NOTICE 'erp_px_cat_g1v2: categories loaded/promoted/enriched';
    ...
    -- ERP CUSTOMER
    ----------------------------------------------------------------------------
    RAISE NOTICE '>> Truncate The Table and insert into silver.erp_cust_az12';
   ...
   RAISE NOTICE '----------';
	...
	SELECT
		...
		TRIM(cst_lastname) AS cst_lastname,
	        ...
	        ELSE 'n/a'
	    END cst_marital_status, -- Normalize the marital status values to readable format
		    CASE 
